...
**The kit catalog is *not* in this repository.** Kits are *content/data*, served from an external catalog supplied at runtime via `KITS_ROOT` (a local checkout in dev, a mounted volume in production). Never add a `kits/` directory or an `instructions/` kit guide here — kit authoring lives in the separate kit-catalog repository. See `.ai/rules.md` for the authoritative rules for working in this repo.

**Intended usage** — kits are selected **per task**, not pinned once per project. An agent reflects on which traits the current task touches (these can emerge mid-conversation — e.g. "add auth" resolving to OIDC after discussion) and loads the matching kits via trait reflection: `list_available_traits`/`list_kits` → `select_kits`/`explain_kit_candidate` → `get_kit_outline` → `get_kit` (the layered tool chain detailed below). Hard-coding a kit list in `CLAUDE.md`/`AGENTS.md` is a last resort, acceptable only when a project's relevant kits are genuinely stable. The server ships this workflow as the FastMCP `instructions` string (`MCP_INSTRUCTIONS` in `app/main.py`), surfaced to clients on connect.
...
# Local dev server (KEYCLOAK_URL/REALM/RESOURCE_BASE_URL are required env vars)
...
docker run -e KITS_ROOT=/data/kits -v "/path/to/your/kit-catalog:/data/kits" ... quartermaster
```
...
the parser/validator must stay consistent with it. The catalog (`KITS_ROOT`) holds:

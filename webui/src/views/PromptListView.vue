...
const nameRule = (v: string) =>
  ...
  'Lowercase words joined by hyphens, e.g. release-notes'
...
async function confirmDelete(): Promise<void> {
  if (deleteTarget.value) {
    await deletePrompt(deleteTarget.value)
  }
...
<template>
  <v-container>
    <SectionHeader
      title="Prompts"
      subtitle="The shared prompts catalog your agents and clients draw from."
      tag="PROMPT_INVENTORY"
    />
    <div class="d-flex align-center mb-4">
      ...
      <v-chip v-else size="small" variant="tonal" color="info">
        Read-only — editor role required to modify prompts
      </v-chip>
    </div>
    ...
    <v-card>
      ...
      >
        ...
        <template #no-data>
          <div class="pa-4 text-medium-emphasis">No prompts yet.</div>
        </template>
      </v-data-table>
    </v-card>

    <v-dialog v-model="createOpen" max-width="640">
      <v-card title="New prompt">
        <v-card-text>
          <v-alert
            v-if="createError"
            type="error"

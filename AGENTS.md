...
- **`app/eval/`** — in-process **catalog-evaluation** of kit resolution quality, for kit authors (`app/eval/README.md`). A portable corpus (one auto-probe per kit derived from its manifest — `requires` = traits that should be inferred, `excludes` = traits that must not — plus a bundled curated language-purity suite in `cases/curated.yaml`) is run through the *real* resolver path: `runner.py` calls `_infer` + `select_kits_v2` directly, **bypassing the `resolve_kits` wrapper** so the memory nudge and metrics attribution never fire (a batch eval has no caller). `report.py` (pure) scores records into per-case verdicts and a **catalog-wide false-exclusion tally** — which kits an over-inferred trait silently drops, the headline harm. Each record carries the `engine` that actually ran, so a silent degrade to the lexical floor shows up as `engine_drift`. Exposed two ways: the `python -m app.eval` CLI (local, no server; CI-gate exit code) and the async API. `app/services/eval_service.py` owns a process-local background-job store (`app/eval/jobs.py`, in-memory, single-process) and `app/routers/eval.py` serves `POST /api/eval/resolution` (202 + job id) / `GET /api/eval/resolution/{id}` (poll → report); `EvalJobNotFoundError` maps to 404.
